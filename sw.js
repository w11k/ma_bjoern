...
self.addEventListener('install', event => {
    ...
    event.waitUntil(
        fetch(indexPage).then(response => caches.open('vanilla-wc').then(cache => {
...
self.addEventListener('fetch', event => {
    const updateCache = request => caches.open('vanilla-wc').then(cache => {
    ...
    event.respondWith(
        fetch(request).catch(error => {
            ...
            return caches.open('vanilla-wc').then(cache => {
